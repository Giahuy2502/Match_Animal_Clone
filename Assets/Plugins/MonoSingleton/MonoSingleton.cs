...
public abstract class MonoSingleton<T> : MonoBehaviour where T : MonoBehaviour
{
    ...
    ///<summary> Return the instance (if exist), or create new instance if it not exist. So better check HasInstance() before call this
    ///to make sure we has intance. And don't call Instance at Disable() or Destroy(), will lead to memoryleak !!</summary>
    public static T Instance
    {
        get
        {
            ...
            {
                Debug.LogWarning("Call Instance, but Application is not running, are you call to any Singleton at OnDestroy?");
                return null;
            }
            ...
            if(s_instance == null)
            {
                ...
                Debug.Log($"Create singleton : {singleton.name}");
            }
            return s_instance;
        }
        private set { }
    ...
    protected void Awake()
    {
        ...
        if(s_instance != null && s_instance.GetInstanceID() != this.GetInstanceID())
        {
            ...
            Debug.LogFormat("An instance of [{0}] already exist : [{1}], So destroy this instance : [{2}]!!", typeof(T).ToString(), s_instance.gameObject.name, name);
            Destroy(gameObject);
        }
    ...
    /// <summary> The extend classes MUST NOT override this function, let the MonoSingleton do the checking stuff here. The 
    /// extend classes should override the DoOnEnable function to do the task that they want to do on Start().
    /// Mark this function as protected, so the compiler will thrown a warning if any extend classes write Start function </summary>
    ...
    protected void OnDestroy()
    {
        ...
        {
            #if UNITY_EDITOR
            Debug.LogFormat("Destroy instance of [{0}]", typeof(T));
            #endif
            DoOnDestroy();
            s_instance = null;
        }
